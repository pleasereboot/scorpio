<?php
	...
	class db {
		...
		function insert($table, $INSERT){	 //3.32 fb 
			...
			$val_column 	= implode(",", $COLS);
			$val_item 		= implode(",", $VALUES);		
		...
		function update($table, $id, $UPDATE_QUERY, $col='id'){	// updated 3.29	
			...
			$query = "UPDATE `$table` SET $set WHERE `$col` = '$id';";
		...
		function table_exist($table){ //WTF
		...
		function select($query, $data_only = true) {
			$SELECT['query'] 			= $query;
