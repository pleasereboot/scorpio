<?php 
	...
	include('contents/mod_store/class_store.php');


	$S = new store();

	$return .= $S->select_items();
	
	$return .= $S->select_items_types();
...
//				$BASKET = new doc($_GET['order_id']);		
//					
//				$BASKET->change_type('Credit note');
//				$BASKET->change_mdate();
...
//				$ORDER->saveas('Invoice');	
//							
//				$INVOICE = new doc($ORDER->new_id);				
//				$INVOICE->change_type('Invoice');
...
//				
//			 // FILL PRODUCTS ADD 
//				if (!in_array($DOC['ITEMS'][$doc_id]['pid'], $NO_EDIT) && is_allowed(5)) {
//					$returnpath  = qs_load();
//									
...
//					$document_html .= ' Créer : ' . url(' quote', qs_load() 			. '&m=baskettoquote&order_id=' 	. $doc_id);
//					$document_html .= ' | ' . 		url(' order', qs_load() 			. '&m=baskettoorder&order_id=' 	. $doc_id);
//					$document_html .= ' | ' . 		url(' booking', qs_load() 			. '&m=baskettobooking&order_id=' 	. $doc_id);
//					$document_html .= ' | ' . 		url(' invoice', qs_load() 			. '&m=baskettoinvoice&order_id=' 	. $doc_id);
//					$document_html .= ' | ' . 		url(' credit note', qs_load() 		. '&m=baskettocredit&order_id=' . $doc_id);
//					$document_html .= ' | ' . 		url(' stock adjustment', qs_load()	. '&m=baskettoadjust&order_id=' . $doc_id) . BR . BR;	
//				}
//	
//			/// IF QUOTE
