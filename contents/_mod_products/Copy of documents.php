	...
	switch($mode){			
		...
		case "ordertoinvoice":	
			...
			e($ORDER_DATA);	
				...
				echo $new_invoice = contents_duplicate($order_id, 'fromorder', $PAR="", $table="prod_documents");
	
				$query = "UPDATE `prod_documents` SET `pid` = '|4326|' WHERE `id` = '$new_invoice';";
				$DB->query($query);	
	
				$query = "UPDATE `prod_documents` SET `document_id` = '' WHERE `id` = '$new_invoice';";
				$DB->query($query);	
				
				$ref_id = $ORDER_DATA['DATA'][$order_id]['document_id'];	
				$query = "UPDATE `prod_documents` SET `ref_id` = '$ref_id' WHERE `id` = '$new_invoice';";
				$DB->query($query);		
...
//									</form>" ;
		...
		/// IF ORDER
			if ($DOC['ITEMS'][$doc_id]['pid'] == '|4573|') {
				...
				//$document_html .= ' | ' . url('Placer la commande', qs_load() 	. '&m=order_confirm&order_id=' 	. $doc_id);
				//$document_html .= ' | ' . url('Ajuster linventaire', qs_load() 	. '&m=adjust_confirm&order_id=' . $doc_id);
				//$document_html .= ' | ' . url('Note de credit', qs_load() 		. '&m=credit_confirm&order_id=' . $doc_id);
			}			
